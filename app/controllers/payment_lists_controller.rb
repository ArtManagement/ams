class PaymentListsController < ApplicationController
  def index
    ...
    sql =
    "SELECT p.id AS id,
       p.payment_slip_id AS payment_slip_id,
       p.artwork_id AS artwork_id,
       ps.date AS payment_date,
       s.name AS supplier,
       ...
       END AS status
     FROM   payments AS p
     LEFT OUTER JOIN payment_cancels AS pc ON p.id = pc.payment_id
     LEFT OUTER JOIN payment_cancel_slips AS pcs ON pc.payment_cancel_slip_id = pcs.id,
       payment_slips AS ps,
       customers AS s,
       artworks AS a
     ...
     AND   p.payment_slip_id = ps.id
     AND   ps.customer_id = s.id"
#     gon.payment_list_data =  Purchase.find_by_sql([sql])
     gon.payment_list_data =  []
  ...
  def search
    ...
    "SELECT p.id AS id,
     ...
     @payment_search_data =  Purchase.find_by_sql([sql,{date_from: date_from, date_to: date_to,
     ...
     render json: @payment_search_data
  end
end
