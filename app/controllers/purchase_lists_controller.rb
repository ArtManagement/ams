class PurchaseListsController < ApplicationController
  def index
    ...
    "SELECT p.id AS id,
     ...
     LEFT OUTER JOIN purchase_cancel_slips AS pcs ON pc.purchase_cancel_slip_id = pcs.id,
     ...
     gon.purchase_list_data =  Purchase.find_by_sql([sql])
  ...
  def search
    ...
    "SELECT p.id AS id,
     ...
     @purchase_search_data =  Purchase.find_by_sql([sql,{date_from: date_from, date_to: date_to,
     ...
     render json: @purchase_search_data
  end
end
