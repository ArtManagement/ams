class ReceiptListsController < ApplicationController
  def index
    ...
    sql =
    "SELECT p.id AS id,
       p.receipt_slip_id AS receipt_slip_id,
       p.artwork_id AS artwork_id,
       ps.date AS receipt_date,
       s.name AS supplier,
       ...
       CASE WHEN sa.price = 0
            ...
            ELSE sa.price
       END AS sale_price,
       CASE WHEN sa.price * ss.tax_rate * ss.tax_class_id = 0
            THEN NULL
            ELSE sa.price * ss.tax_rate * ss.tax_class_id
       END AS sale_tax,
       p.note AS note,
       CASE WHEN ss.id IS NOT NULL
            THEN '売上'
            ELSE CASE WHEN pcs.id IS NOT NULL
                      THEN '返品'
                 END
       END AS status
     FROM   receipts AS p
     LEFT OUTER JOIN receipt_cancels AS pc ON p.id = pc.receipt_id
     LEFT OUTER JOIN receipt_cancel_slips AS pcs ON pc.receipt_cancel_slip_id = pcs.id,
       receipt_slips AS ps,
       customers AS s,
       artworks AS a
     ...
     AND   p.receipt_slip_id = ps.id
     AND   ps.customer_id = s.id"
#     gon.receipt_list_data =  Purchase.find_by_sql([sql])
     gon.receipt_list_data =  []
  ...
  def search
    ...
    "SELECT p.id AS id,
     ...
     @receipt_search_data =  Purchase.find_by_sql([sql,{date_from: date_from, date_to: date_to,
     ...
     render json: @receipt_search_data
  end  
end
