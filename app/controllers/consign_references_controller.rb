class ConsignReferencesController < ApplicationController
  def index
    ...
    "SELECT
     ...
     AND  s.id IS NULL
     AND  c.id IS NULL "
     gon.consign_reference =  Consign.find_by_sql([sql])
  end
  ...
  def search
    ...
    "SELECT
     ...
     @consign_reference_search =  Consign.find_by_sql([sql,{artist_id_from: artist_id_from, artist_id_to: artist_id_to,
                                                              title: title,
     ...
     render json: @consign_reference_search
  end

  def select
    @consign = Consign.new
    @consign.artwork_id = params[:artwork_id]
    @consign.consign_slip_id = params[:slip_id]
    @consign.save
    sql =
    "SELECT
       a.id AS id,
