class ExhibitReferencesController < ApplicationController
  def index
    ...
    "SELECT
       ...
       END AS size,
       CASE WHEN (a.technique_id IS NOT NULL) AND (te.technique IS NOT NULL)
            THEN te.technique
            ELSE ca.category
       END AS category,
       f.format AS formats,
       CASE WHEN t.price = 0
            ...
            ELSE t.price
       END AS trust_price,
       CASE WHEN t.price * ts.tax_rate * ts.tax_class_id = 0
            THEN NULL
            ELSE t.price * ts.tax_rate * ts.tax_class_id
       END AS trust_tax,
       ps.slip_no AS purchase_slip_no,
       ts.slip_no AS trust_slip_no,
       cs.slip_no AS consign_slip_no
     FROM
       artworks AS a
       ...
       LEFT OUTER JOIN exhibits AS e ON a.id = e.artwork_id
         LEFT OUTER JOIN exhibit_slips AS es ON e.exhibit_slip_id = es.id
     ...
     AND s.id IS NULL "
     gon.exhibit_reference =  Exhibit.find_by_sql([sql])
  end
  ...
  def search
    ...
    "SELECT
     ...
     @exhibit_reference_search =  Exhibit.find_by_sql([sql,{artist_id_from: artist_id_from, artist_id_to: artist_id_to,
                                                              title: title,
     ...
     render json: @exhibit_reference_search
  end

  def select
    @exhibit = Exhibit.new
    @exhibit.artwork_id = params[:artwork_id]
    @exhibit.exhibit_slip_id = params[:slip_id]
    @exhibit.save
    sql =
    "SELECT
       a.id AS id,
